...
class BuckleyLeverette(object):
    def __init__(self, nx=100, length=1.0, Swc=0.2, Sor=0.2, mu_w=1.0, mu_o=5.0, dt=0.01, total_time=3.0):
        self.nx = nx
        self.length = length
        self.dx = length / (nx - 1)
    ...
    def simulate_at_k(self, Sw, qtk):
        ...
        fw_right = np.roll(fw, -1)
        
        # Upwind scheme
        dSw = -(self.dt / self.dx) * qtk *(fw_right - fw)
        Sw[1:] += dSw[0:-1]
        Sw = np.clip(Sw, self.Swc, 1.0)
    ...
    def stage_cost(self, Swk, qtk):
        ...
        return qtk*((1-fwN)*ro - rwp*fwN - rwi)
